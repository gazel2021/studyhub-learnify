      ...
      <Tabs defaultValue={canSettings ? "brand" : canPages ? "pages" : canTax ? "tax" : "admins"}>
        <TabsList className="glass rounded-full p-1 mb-6 flex-wrap h-auto">
          {canSettings && (
            <TabsTrigger
              value="brand"
              className="rounded-full data-[state=active]:bg-gradient-neon data-[state=active]:text-white"
            >
            ...
            <div className="glass rounded-3xl p-6 md:p-8 space-y-6">
              <div>
                ...
                <Input
                  ...
                  className="h-11 rounded-xl mt-1.5 max-w-md bg-background/50"
                />
              </div>
              <div>
                ...
                <div className="flex items-center gap-5">
                  {s.logoUrl ? (
                    <img
                      src={s.logoUrl}
                      alt="logo"
                      className="h-20 w-20 rounded-2xl object-contain bg-background/50 p-2"
                    />
                  ) : (
        ...
        {canTax && (
          <TabsContent value="tax">
            <div className="grid gap-4 md:grid-cols-2">
              <TaxonomyCard
                title={t("settings.add.subject")}
                icon={BookOpen}
                items={[
                  ...SUBJECTS.map((x) => ({
                    id: x.key,
                    label: t(`subject.${x.key}`),
                    builtin: true,
                  })),
                  ...s.customSubjects.map((x) => ({
                    id: x.key,
                    label: x.label.ar,
                    builtin: false,
                  })),
                ...
                onAdd={(k, ar, en, fr) => {
                  if (!s.addSubject(k, ar, en, fr)) toast.error(t("settings.exists"));
                  else toast.success("✓");
                }}
              ...
              />
              <TaxonomyCard
                title={t("settings.add.country")}
                icon={Globe}
                items={[
                  ...COUNTRIES.map((c) => ({ id: c, label: t(`country.${c}`), builtin: true })),
                  ...s.customCountries.map((x) => ({
                    id: x.code,
                    label: x.label.ar,
                    builtin: false,
                  })),
                ...
                onAdd={(k, ar, en, fr) => {
                  if (!s.addCountry(k, ar, en, fr)) toast.error(t("settings.exists"));
                  else toast.success("✓");
                }}
              ...
              <TaxonomyCard
                ...
                onAdd={(k, ar, en, fr) => {
                  if (!s.addStage(k, ar, en, fr)) toast.error(t("settings.exists"));
                  else toast.success("✓");
                }}
              ...
              <TaxonomyCard
                ...
                onAdd={(k, ar, en, fr) => {
                  if (!s.addType(k, ar, en, fr)) toast.error(t("settings.exists"));
                  else toast.success("✓");
                }}
  ...
  return (
    <div className="glass rounded-3xl p-5">
      <div className="flex items-center gap-2 mb-3">
        ...
        <Link
          to={`/${slug}` as "/privacy"}
          className="text-xs text-muted-foreground hover:text-foreground ms-auto underline-offset-4 hover:underline"
        >
          {t("section.viewAll")} →
        </Link>
      </div>
      <Textarea
        ...
        className="font-mono text-sm rounded-xl bg-background/50"
      />
      <Button
        onClick={() => {
          s.setPage(slug, body);
        ...
        className="mt-3 h-10 rounded-full bg-gradient-neon text-white font-bold"
      >
  ...
  return (
    <div className="glass rounded-3xl p-5">
      ...
      <div className="grid grid-cols-2 gap-2 mb-2">
        <Input
          placeholder={t(fields[0] === "code" ? "settings.field.code" : "settings.field.key")}
          value={k}
          onChange={(e) => setK(e.target.value)}
          className="h-9 rounded-lg bg-background/50 col-span-2"
        />
        <Input
          placeholder={t("settings.field.ar")}
          value={ar}
          onChange={(e) => setAr(e.target.value)}
          className="h-9 rounded-lg bg-background/50 col-span-2"
        />
        <Input
          placeholder={t("settings.field.en")}
          value={en}
          onChange={(e) => setEn(e.target.value)}
          className="h-9 rounded-lg bg-background/50"
        />
        <Input
          placeholder={t("settings.field.fr")}
          value={fr}
      ...
      <Button
        size="sm"
        className="rounded-full bg-gradient-neon text-white font-bold w-full"
        onClick={() => {
          if (!k.trim() || !ar.trim()) return;
          onAdd(k, ar, en, fr);
          setK("");
          setAr("");
          setEn("");
      ...
      <div className="mt-4 flex flex-wrap gap-1.5 max-h-40 overflow-auto">
        {items.map((it) => (
          <span
            key={it.id}
            className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-[11px] border ${it.builtin ? "bg-white/5 border-white/10 text-muted-foreground" : "bg-[oklch(0.68_0.22_255)]/10 border-[oklch(0.68_0.22_255)]/30 text-foreground"}`}
          >
            {it.label}
            {!it.builtin && (
              <button
    ...
    <div className="grid gap-6 md:grid-cols-2">
      <form onSubmit={submit} className="glass rounded-3xl p-6 space-y-4">
        ...
        <div className="space-y-2 pt-2">
          {ALL_PERMISSIONS.map((p) => (
            <label
              key={p}
              className="flex items-center gap-2 cursor-pointer hover:bg-white/5 rounded-lg p-2 transition-smooth"
            >
        ...
        <Button
          type="submit"
          className="w-full h-11 rounded-full bg-gradient-neon text-white font-bold"
        >
      ...
      <div className="glass rounded-3xl p-6">
        <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
          <Shield className="h-5 w-5 text-[oklch(0.66_0.24_295)]" />
          {t("settings.admins")}
        </h3>
              ...
              <div className="flex items-center justify-between mb-2">
                <div>
                  <div className="font-bold flex items-center gap-2">
                    {a.name}
                    {a.isOwner && (
                      <span className="text-[10px] uppercase tracking-wider text-amber-300 bg-amber-500/15 border border-amber-400/30 rounded-full px-2 py-0.5">
                        {t("settings.user.owner")}
                      </span>
                    )}
                  </div>
                ...
                {!a.isOwner && (
                  <Button
                    ...
                    onClick={() => {
                      if (confirm(t("admin.confirm"))) removeUser(a.id);
                    }}
                  >
                    ...
                    <button
                      ...
                      onClick={() => {
                        const next = has
                          ...
                          : [...a.permissions, p];
                        setPerms(a.id, next);
                        toast.success(t("settings.permsUpdated"));
                      }}
...
function UsersManager() {
  ...
  return (
    <div className="glass rounded-3xl p-5">
      <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
        <UserCog className="h-5 w-5 text-[oklch(0.66_0.24_295)]" />
      ...
      <div className="space-y-2">
        {users.map((u) => (
          <div
            key={u.id}
            className="flex flex-wrap items-center gap-3 p-3 rounded-2xl bg-background/40 border border-white/5"
          >
            <div className="h-10 w-10 rounded-xl bg-gradient-neon flex items-center justify-center font-bold text-white">
              {u.name.charAt(0)}
            </div>
            <div className="flex-1 min-w-[180px]">
              <div className="font-bold text-sm">
                {u.name}{" "}
                {u.disabled && (
                  <span className="ms-2 text-[10px] text-rose-300 bg-rose-500/15 border border-rose-400/30 rounded-full px-2 py-0.5">
            ...
            <select
              ...
              className="h-9 px-3 rounded-lg bg-background/50 text-sm border border-white/10"
            >
            ...
            <Button
              ...
              className="text-rose-400 hover:bg-rose-500/10"
              onClick={() => {
                if (confirm(t("admin.confirm"))) removeUser(u.id);
              }}
            >
              <Trash2 className="h-4 w-4" />
            </Button>
