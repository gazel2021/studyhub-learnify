...
import { PRODUCTS, SUBJECTS, COUNTRIES } from "@/lib/data";
import { Input } from "@/components/ui/input";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { Button } from "@/components/ui/button";

type Search = { subject?: string; country?: string; q?: string };
...
function ProductsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return list;
  }, [q, subject, country, sort]);

  return (
    <div className="mx-auto max-w-7xl px-4 md:px-6 py-10">
      <div className="mb-8">
        <h1 className="text-4xl font-bold">Browse resources</h1>
        <p className="mt-2 text-muted-foreground">Discover {PRODUCTS.length}+ books, exams and quizzes.</p>
      </div>

      <div className="glass rounded-2xl p-4 mb-8 grid gap-3 md:grid-cols-[1fr_180px_180px_160px]">
        ...
        <Select value={country} onValueChange={(v) => { setCountry(v); navigate({ search: { ...search, country: v === "all" ? undefined : v } }); }}>
          <SelectTrigger className="h-11 rounded-xl"><SelectValue placeholder="Country" /></SelectTrigger>
          <SelectContent>
        ...
        <Select value={sort} onValueChange={setSort}>
          <SelectTrigger className="h-11 rounded-xl"><SlidersHorizontal className="h-4 w-4 mr-1" /><SelectValue /></SelectTrigger>
          <SelectContent>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-20">
          ...
          <Button variant="outline" className="mt-4" onClick={() => { setQ(""); setSubject("all"); setCountry("all"); navigate({ search: {} }); }}>Reset filters</Button>
        </div>
      ) : (
