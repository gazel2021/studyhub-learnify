...
export const Route = createFileRoute("/privacy")({ component: () => <Page slug="privacy" /> });

function Page({ slug }: { slug: "privacy" | "contact" | "support" | "terms" }) {
  const t = useT();
  const body = useSettings((s) => s.pages[slug].body);
  return (
    <div className="mx-auto max-w-3xl px-4 md:px-6 py-12">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-3xl md:text-5xl font-bold font-display tracking-tight mb-6">{t(`page.${slug}.title`)}</h1>
